...
import "./style.scss";
import { IDimensions, TrackedPoint2D } from "./types";
...
const main = async (
  ...
  const interactionArea = document.getElementById(
    "interaction-area"
  ) as HTMLDivElement;
...
const connect = async (
  ...
  parsedOutputDimensions: IDimensions
) => {
  console.log("Connect Tether @ ", tetherHost, "...");

  const agent = await TetherAgent.create("trackingSim", {
    protocol: "ws",
    host: tetherHost || undefined,
    port: 15675,
    path: "/ws",
  });
  main(
