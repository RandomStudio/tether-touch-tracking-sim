<script lang="ts">
	...
	const remapCoordsFromOrigin = (inC: [number, number]): [number, number] => {
		...
		switch (originMode) {
			case 'TOP_LEFT': {
				const [x, y] = remapCoords([inX, inY], inputDimensions, outputDimensions);
				return [x, y];
			}
			case 'TOP_CENTRE': {
				return [
					remap(inX, [0, inputDimensions[0]], [-outputDimensions[0] / 2, outputDimensions[0] / 2]),
			...
			case 'CENTRE_CENTRE': {
				return [
					remap(inX, [0, inputDimensions[0]], [-outputDimensions[0] / 2, outputDimensions[0] / 2]),
					remap(inY, [0, inputDimensions[1]], [-outputDimensions[1] / 2, outputDimensions[1] / 2])
				];
			}
		}
	};
	...
	onMount(async () => {
		...
		if (dimensionsParams) {
			const [width, height] = dimensionsParams.split(',').map((i) => parseFloat(i));
